    ...
    <div className="card p-6 relative overflow-hidden">
      ...
      <div className="relative flex flex-col md:flex-row md:items-start gap-8">
        ...
        <div className="flex-1">
          <p className="text-xs font-medium text-[hsl(var(--muted-foreground))] uppercase tracking-widest mb-2">
            Net Worth
          </p>
          <div className="flex items-baseline gap-3 flex-wrap">
            <span className="font-display text-4xl md:text-5xl text-[hsl(var(--foreground))] tracking-tight num">
              ₹{formatCompact(summary.net_worth)}
            </span>
            {change !== 0 && (
              <div className={`flex items-center gap-1 text-sm font-medium ${isPositive ? 'text-gain' : isNegative ? 'text-loss' : 'text-neutral'}`}>
                ...
                <span className="text-xs opacity-75">({changePct > 0 ? '+' : ''}{changePct.toFixed(1)}%)</span>
              </div>
            )}
          </div>
          {prevSnapshot && (
            <p className="text-xs text-[hsl(var(--muted-foreground))] mt-1.5">vs last month</p>
          )}
        </div>
